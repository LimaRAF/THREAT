...
# 
# #############################################
# #### PREPARING AND ADDING INVENTORY DATA ####
# #############################################
# 
# ##Loading functions
# source("data/data-raw/references_data_prep.R")
# #source(paste(path,"abundances_data_prep.R",sep="/"))
# 
# ## EDITING TREECO SITE AND ABUNDANCE DATA ##
# #Loading the data
# surveys = read.csv("data/data-raw/references.csv",as.is=TRUE, encoding = "UTF-8")
# trees = read.csv("data/data-raw/abundances.csv",as.is=TRUE, na.string=c(""," ",NA))
# 
# 
...
# keep.cols <- c("ordem", "RecordID", "species.correct", "family",
#                "taxon.rank", "tax_check", "notes", "accession",
#                "coletor", "number", "record", "status.spLink",
#                "DetBy", "DetDate",
#                "lat1", "long1", "confiabilidade",
#                "domain","Unidade_de_conservacao", "year_data", "year","forest_type",
#                "country","state","county","locality","locality1")
# trees.final1 <- trees.final[, keep.cols]
# ## Saving
# saveRDS(trees.final1, "data/treeco_inventory_records.rds", compress = "gzip")
