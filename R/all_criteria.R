...
all.crit$category[!is.na(all.crit$only.type) & !is.na(all.crit$last.record) &
           all.crit$only.type & all.crit$last.record <1970 & all.crit$category %in% "CR"] <- "CR_PE"
rm(MyData); gc()
...
reflora <- readRDS("data/threat_flora_info.rds") # information from ReFlora
tmp <- merge(all.crit, reflora[,c("species.correct2","endemismo","dominioFitogeografico")], 
             by.x = "species", by.y = "species.correct2", all.x = TRUE)
table(tmp$species == all.crit$species)
all.crit <- cbind.data.frame(all.crit,
                             tmp[,c("endemismo","dominioFitogeografico"), drop = FALSE], stringsAsFactors = FALSE)
...
tmp6 <- merge(all.crit, 
              tmp5, by.x = "species", by.y = "FullName", all.x = TRUE, sort= FALSE)
tmp6 <- tmp6[!duplicated(tmp6$species),]
tmp6 <- tmp6[order(tmp6$species),]
table(tmp6$species == all.crit$species)
all.crit <- cbind.data.frame(all.crit,
                             tmp6[,c("CurrentListing","AnnotationEnglish","X.AnnotationSymbol"), drop = FALSE], stringsAsFactors = FALSE)
names(all.crit)[(dim(all.crit)[2]-2):dim(all.crit)[2]] <- 
  c("CITES.CurrentListing","CITES.AnnotationEnglish","CITES.AnnotationSymbol")
...
prev.assess <- readRDS("data/previous_assessments_spp.rds")
