...
## Convex Hull method
# IUCN guidelines suggest to not crop the EOO for criterion B
system.time(
  EOO.hull <- my.EOO.computing(MyData[grepl("high", MyData$tax.check.final), c(1:3)], 
                            method = "convex.hull",
...
shps1 <- shps[!is.na(shps)]
shps1 <- do.call(rbind, shps1)
shps_df <- SpatialPolygonsDataFrame(shps1, data.frame(tax = names(shps1), row.names = names(shps1)))
#shps_df <-EOO.hull[[2]]
...
system.time(
  EOO.hull.sensitivity <- my.EOO.sensitivity (MyData[, c(1:3,10)],
...
EOO.hull.sensitivity1 <- my.EOO.sensitivity (MyData[, c(1:3,9)],
...
radius <- my.subpop.radius(MyData[grepl("high", MyData$tax.check.final), c(1:3)], 
                       factor.div = 10, quant.max = 0.9)
radius1 <- my.subpop.radius(MyData[, c(1:3)], 
                        factor.div = 10, quant.max = 0.9)
radius2 <- my.subpop.radius(MyData[MyData$tax.check2, c(1:3)], 
                        factor.div = 10, quant.max = 0.9)
...
SUB <- ConR::subpop.comp(MyData[grepl("high", MyData$tax.check.final), c(1:3)], # optimum tax. confidence
                   Resol_sub_pop= radius.new[,c("tax", "radius")],
                   parallel = TRUE, NbeCores = 5)
SUB1 <- ConR::subpop.comp(MyData[, c(1:3)],
                    Resol_sub_pop= radius.new1[,c("tax", "radius")], # any confidence
                    parallel = TRUE, NbeCores = 5)
SUB2 <- ConR::subpop.comp(MyData[MyData$tax.check2, c(1:3)],
...
system.time( # Took using v1.3
  AOO <- my.AOO.computing(MyData[!is.na(MyData$ddlat) & !is.na(MyData$ddlon) & grepl("high", MyData$tax.check.final), c(1:3)], 
...
system.time(
  AOO1 <- my.AOO.computing(MyData[!is.na(MyData$ddlat) & !is.na(MyData$ddlon), c(1:3)], 
                       Cell_size_AOO = 2, # Size of the grid cell in kilometers
                       nbe.rep.rast.AOO = 30, # number of raster with random starting position for estimating the AOO
...
system.time(
  AOO2 <- my.AOO.computing(MyData[!is.na(MyData$ddlat) & !is.na(MyData$ddlon) & MyData$tax.check2, c(1:3)], 
                       Cell_size_AOO = 2, # Size of the grid cell in kilometers
                       nbe.rep.rast.AOO = 30, # number of raster with random starting position for estimating the AOO
                       parallel = TRUE, NbeCores = 6, 
                       show_progress= TRUE, export_shp=FALSE)
...
# For not consuming too much of the local HD
write("TMPDIR = 'D:/Rtemp'", file=file.path(Sys.getenv('R_USER'), '.Renviron'))
...
axis(1, at=c(0.5,1.5,2.5,3.5,4.5,5.5), labels = c("0 km", "25 km","50 km", "75 km", "100 km", "Est. radius"))
